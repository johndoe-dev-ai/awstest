...
def copy_to_deep_archive(source_bucket, object_key, target_bucket, storage_class):
    """
    Copy an object to the target bucket with specified storage class.
    """
    try:
        ...
        print(f"Successfully copied {object_key} to {target_bucket} with {storage_class}")
        return True
    except ClientError as e:
        print(f"Error copying object {object_key}: {e}")
        return False

def delete_object(source_bucket, object_key):
    """
    Delete an object from the source bucket.
    """
    try:
        s3_client.delete_object(Bucket=source_bucket, Key=object_key)
        print(f"Successfully deleted {object_key} from {source_bucket}")
    except ClientError as e:
    ...
    for config in configs:
        try:
            ...
            for obj in objects_to_archive:
                if copy_to_deep_archive(bucket, obj['Key'], target_bucket, storage_class):
                    successfully_copied.append(obj['Key'])

            for key in successfully_copied:
